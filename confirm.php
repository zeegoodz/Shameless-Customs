...
if($action['result'] != 'error'){
	...
	$check_key = mysql_query("SELECT * FROM verify WHERE `email` = '$email' AND `key` = '$key' LIMIT 1") or die(mysql_error());
	...
	if(mysql_num_rows($check_key) != 0){
		...
		$update_users = mysql_query("UPDATE user SET `active` = 1 WHERE `id` = '$confirm_info[userid]' LIMIT 1") or die(mysql_error());
		//delete the confirm row
		$delete = mysql_query("DELETE FROM verify WHERE `id` = '$confirm_info[id]' LIMIT 1") or die(mysql_error());
